...
def functional_conv_block(x: torch.Tensor, weights: torch.Tensor, biases: torch.Tensor,
    ...
    x = F.conv1d(x, weights, biases, padding=1)
    x = F.batch_norm(x, running_mean=None, running_var=None, weight=bn_weights, bias=bn_biases, training=True)
    x = F.relu(x)
...
class FewShotClassifier(nn.Module):
    def __init__(self, num_input_channels: int, k_way: int, final_layer_size: int = 64):
        ...
        print("models.py is now performing few shot classification (convolutional 1d x 4 layer model)")
    ...
    def functional_forward(self, x, weights):
        """Applies the same forward pass using PyTorch functional operators using a specified 
        of weights."""
        print("\n\n\nX", x.shape)
        print("weights 1.0", weights['conv1.0.weight'].shape)
        print("bias 1.0", weights['conv1.0.bias'].shape)        
        print("weights 1.1", weights['conv1.1.weight'].shape)
        print("bias 1.1", weights['conv1.1.bias'].shape)
        
        print("weights 2.0", weights['conv2.0.weight'].shape)
        print("bias 2.0", weights['conv2.0.bias'].shape)        
        print("weights 2.1", weights['conv2.1.weight'].shape)
        print("bias 2.1", weights['conv2.1.bias'].shape)        

        print("weights 3.0", weights['conv3.0.weight'].shape)
        print("bias 3.0", weights['conv3.0.bias'].shape)        
        print("weights 3.1", weights['conv3.1.weight'].shape)
        print("bias 3.1", weights['conv3.1.bias'].shape)        
        
        
        print("weights 4.0", weights['conv4.0.weight'].shape)
        print("bias 4.0", weights['conv4.0.bias'].shape)        
        print("weights 4.1", weights['conv4.1.weight'].shape)
        print("bias 4.1", weights['conv4.1.bias'].shape,"\n\n")        
        ...
        x = x.view(x.size(0), -1)
        print("x", x.shape)
        print("weights['logits.weight']", weights['logits.weight'].shape)
        print("weights['logits.bias']", weights['logits.bias'].shape)
...
class MatchingNetwork(nn.Module):
    def __init__(self, n: int, k: int, q: int, fce: bool, num_input_channels: int,
        ...
        print("Creating Matching Network",num_input_channels)
        self.n = n
        self.k = k
        self.q = q
        self.fce = fce
